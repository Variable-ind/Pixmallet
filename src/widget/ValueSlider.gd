...
@export var modulate_icon_color :Color = Color.WHITE:  # change icon color
	set(color): 
		modulate_icon_color = color
		_value_up_button.modulate = color
		_value_down_button.modulate = color

var state := NORMAL
var arrow_is_held := 0  # Used for arrow button echo behavior. Is 1 ValueUp, -1 ValueDown.
...
func _ready() -> void:
	...
	set_process_input(!global_increment_action.is_empty() and \
					  !global_decrement_action.is_empty())
	_reset_display(true)
	_setup_nodes()
...
func _gui_input(event: InputEvent) -> void:
	...
	if state == NORMAL:
		...
		if (event is InputEventMouseButton and event.pressed == false and \
			event.button_index == MOUSE_BUTTON_LEFT):
			state = NORMAL
		...
		if event is InputEventMouseMotion:
			# When pressing/releasing Shift, 
			# reset starting values to prevent slider jumping around
			if get_meta("shift_pressed") != event.shift_pressed:
				set_meta("mouse_start_position", get_local_mouse_position())
				set_meta("start_ratio", ratio)
				set_meta("start_value", value)
				set_meta("shift_pressed", event.shift_pressed)
			var x_delta: float = get_local_mouse_position().x - \
								 get_meta("mouse_start_position").x
...
#		texture_over = get_theme_icon("texture_over", "ValueSlider")
		...
		if editable:
			...
			_line_edit.add_theme_color_override("font_color_uneditable",
												 line_edit_disabled_col)
