...
func generate_image(img: Image, 
					shader: Shader, 
					params: Dictionary, 
					size: Vector2i):
	# duplicate shader before modifying code 
	# to avoid affecting original resource
	shader = shader.duplicate()
	shader.code = shader.code.replace("unshaded",
									  "unshaded, blend_premul_alpha")
	...
	RenderingServer.canvas_item_add_texture_rect(ci_rid, rect, texture)
