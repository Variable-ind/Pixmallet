	...
	func as_dict(big_endian := true) -> Dictionary:
		if big_endian:
			return {
				...
				"node": (_uuid[10] << 40) + (_uuid[11] << 32) + (_uuid[12] << 24) +\
						(_uuid[13] << 16) + (_uuid[14] << 8 ) + _uuid[15]
			}
		else:
			return {
				"low": _uuid[0] + (_uuid[1]  << 8 ) + (_uuid[2]  << 16) +\
					  (_uuid[3] << 24),
				"mid": _uuid[4] + (_uuid[5]  << 8 ),
				"hi": _uuid[6] + (_uuid[7]  << 8 ),
				"clock": _uuid[8] + (_uuid[9]  << 8 ),
				"node": _uuid[10] + (_uuid[11] << 8 ) + (_uuid[12] << 16) +\
						(_uuid[13] << 24) + (_uuid[14] << 32) + (_uuid[15] << 40)
			}
